...
*/
  //UI展示部分
  function AdvertView(target,option) {
  ...
  AdvertView.prototype = {
  	constructor:AdvertView,
        ...
        var    arr = this.config.imageArray,
             ...
             if(href.indexOf("http") == 0) {
                 strContent += "<li data-Index='"+i+"'><a target='_blank' href='"+href+"' data-AdIndex= '"+(Number(arr[i].AdIndex)+1)+"' data-ImageID='"+arr[i].ImageID+"'><img src='"+src+"' /></a></li>";
             } else {
                 strContent += "<li data-Index='"+i+"'><a href='"+href+"' data-AdIndex= '"+(Number(arr[i].AdIndex)+1)+"' data-ImageID='"+arr[i].ImageID+"'><img src='"+src+"' /></a></li>";
             }
...
//业务逻辑部分,输出最终数据
 ...
 AdvertModel.prototype = {

   constructor: AdvertModel,
   ...
   doBuiltProxy: function(arr) {
    ...
    var temp = this.targetSource.splice(pos);

    for( var j = temp.length ; j >= 0 ; j-- ) {//删除undefined的对象
   ...
   doInSertDefaultPisitoinData: function() {
    ...
    for(var k = this.targetSource.length; k >= 0; k--) {
 ...
 //控制模块
 function AdvertController(model,view,updateTime){ 
    this._model = model;
    this._view = view;
